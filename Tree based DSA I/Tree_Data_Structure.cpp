...
/*
	...
	- Tree Terminologies:
		...
		6. Height of a Tree 
			- The height of a Tree is the height of the root node or the depth of
			the deepest node.
		7. Degree of a Node
	...
	- Tree Traversal
		...
		specific node. The tree traversal algorithm helps in visiting a required
		node in the tree.
		
	- Tree Applications
		1. Binary Search Tree(BSTs) are used to quickly check whether an element 
		is present in a set or not.
		2. Heap is a kind of tree that is used for heap sort.
		3. A modified version of a tree called Tries is used in modern routers to 
		store routing information.
		4. Most popular databases use B-Trees and T-Trees, which are variants of
		the tree structure we learned above to store their data.
		5. Compilers use a syntax tree to validate the syntax of every program you 
		write.
